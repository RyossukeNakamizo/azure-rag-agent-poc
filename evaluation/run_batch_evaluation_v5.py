...
def run_rag_pipeline(question: str):
    ...
    groundedness_score = safe_evaluate(evaluate_groundedness, answer=answer_text, context=context_str)
    coherence_score = safe_evaluate(evaluate_coherence, answer=answer_text)
    relevance_score = safe_evaluate(evaluate_relevance, question=question, answer=answer_text)
    return {
        "question": question,
...
def main():
    ...
    print(f"=== RAG Batch Evaluation (v5) ===")
    ...
    output = f"results/d20_batch_v5_{timestamp}.json"
    ...
    if success > 0:
        print(f"\n{'='*60}")
        targets = [
            ("Groundedness", avg_scores.get("groundedness", 0), 0.85),
            ("Coherence", avg_scores.get("coherence", 0), 0.75),
            ("Relevance", avg_scores.get("relevance", 0), 0.80)
        ]
