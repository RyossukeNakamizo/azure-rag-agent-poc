#!/usr/bin/env python3
"""
D22-2: Query Expansion効果の評価スクリプト
D21-4（batch_evaluation_v8.py）との公平比較設計

評価条件:
- Baseline: use_query_expansion=False (D21-4再現)
- Query Expansion: use_query_expansion=True (新機能)

目標:
- Relevance: 0.780 → 0.850 (+9%改善)
- Groundedness: 0.820以上維持
- Coherence: 0.980以上維持
"""
import os
import json
...
def check_api_health():
    ...
    try:
        ...
        print(f"   確認: uvicorn app.main:app --reload で起動していますか？")
        return False
...
"""
    ...
    response = openai_client.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "あなたは公平で厳格な評価者です。基準に従って正確に評価してください。"},
    ...
    # 1-5スケールを0-1に正規化
    return {
        "coherence": scores.get("coherence", 3) / 5.0,
        "relevance": scores.get("relevance", 3) / 5.0,
        "groundedness": scores.get("groundedness", 3) / 5.0
    }
...
def save_results(results: Dict):
    ...
    with open(summary_file, "w", encoding="utf-8") as f:
        ...
        for name, data in results.items():
            ...
            f.write(f"  サンプル数:    {data['metrics']['num_samples']}\n\n")
...
def display_summary(results: Dict):
    ...
    for name, data in results.items():
        print(f"\n【{name}】")
        print(f"  Coherence:    {data['metrics']['coherence_avg']:.3f}")
        print(f"  Relevance:    {data['metrics']['relevance_avg']:.3f}")
        print(f"  Groundedness: {data['metrics']['groundedness_avg']:.3f}")
    ...
    if "baseline" in results and "query_expansion" in results:
        ...
        print("="*70)
        print(f"  Coherence変化:    {(expansion['coherence_avg'] - baseline['coherence_avg'])*100:+.1f}%")
        print(f"  Relevance改善:    {(expansion['relevance_avg'] - baseline['relevance_avg'])*100:+.1f}%")
        print(f"  Groundedness変化: {(expansion['groundedness_avg'] - baseline['groundedness_avg'])*100:+.1f}%")
        
        # 判定
        print("\n【判定】")
        if expansion['relevance_avg'] >= 0.850:
            if expansion['groundedness_avg'] >= 0.800 and expansion['coherence_avg'] >= 0.980:
                print("  ✅ 成功: Query Expansion採用推奨")
            else:
                print("  ⚠️  条件付き成功: 品質メトリクス改善が必要")
        else:
            print("  ❌ 目標未達: Pattern B/C検討またはSemantic Ranker検討")
